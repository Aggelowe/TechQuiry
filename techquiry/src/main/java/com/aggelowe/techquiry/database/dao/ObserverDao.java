...
import static com.aggelowe.techquiry.database.DatabaseConstants.OBSERVER_SELECT_INQUIRY_SCRIPT;
import static com.aggelowe.techquiry.database.DatabaseConstants.OBSERVER_SELECT_USER_SCRIPT;
...
public final class ObserverDao {
	 ...
	 */
	public static List<Observer> selectFromInquiry(int inquiryId) {
		LOGGER.debug("Getting observers with inquiry id " + inquiryId);
		List<PreparedStatement> statements = DatabaseUtilities.loadStatements(Database.getConnection(), OBSERVER_SELECT_INQUIRY_SCRIPT);
		if (statements.size() < 1) {
			throw new DaoException("Invalid number of statements in " + OBSERVER_SELECT_INQUIRY_SCRIPT + "!");
		}
		PreparedStatement statement = statements.getFirst();
		ResultSet result = DatabaseUtilities.executeStatement(statement, inquiryId);
		if (result == null) {
			throw new DaoException("The first statement in " + OBSERVER_SELECT_INQUIRY_SCRIPT + " did not yeild results!");
		}
	 ...
	 * @return The observers with the given id
	 */
	public static List<Observer> selectFromUser(int userId) {
		LOGGER.debug("Getting observers with user id " + userId);
		List<PreparedStatement> statements = DatabaseUtilities.loadStatements(Database.getConnection(), OBSERVER_SELECT_USER_SCRIPT);
		if (statements.size() < 1) {
			throw new DaoException("Invalid number of statements in " + OBSERVER_SELECT_USER_SCRIPT + "!");
		}
		PreparedStatement statement = statements.getFirst();
		ResultSet result = DatabaseUtilities.executeStatement(statement, userId);
		if (result == null) {
			throw new DaoException("The first statement in " + OBSERVER_SELECT_USER_SCRIPT + " did not yeild results!");
		}
		List<Observer> list = new ArrayList<>();
		try {
			while (result.next()) {
